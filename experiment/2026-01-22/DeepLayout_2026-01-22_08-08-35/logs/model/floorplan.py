...
class FloorPlan():
    ...
    def get_layout_image(self,tensor=True):
        ...
        # Background should be outside valid room indices - using 5 as "external"
        vocab = get_vocab()
        num_classes = len(vocab['object_idx_to_name'])
        background_idx = num_classes  # One beyond valid indices (e.g., 5 for 5 classes)
        boundary_idx = num_classes + 1  # One more for boundary (e.g., 6)
        ...
        cv2.polylines(img, (boundary//2).astype(np.int32).reshape(1, -1, 2), True, boundary_idx)
