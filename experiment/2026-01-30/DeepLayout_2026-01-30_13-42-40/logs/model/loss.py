...
def mutex_loss(boxes,step=2):
    ...
    denominator = max(B*FP-B, 1)
    return (f_b_dist*f_out_box).sum()/denominator
...
class CoverageLoss(nn.Module):
    ...
    def _coverage_loss(self,boxes,fragments):
        ...
        return (f_b_dist*f_out_box).min(-1)[0].sum()/denominator   
...
class MutexLoss(nn.Module):
    ...
    def _mutex_loss(self,boxes):
        ...
        return (f_b_dist*f_in_box).sum()/denominator     
...
class BoxRenderLoss(nn.Module):
    ...
    def _render_loss(self,boxes,targets):
        ...
        return ((b_t_dist*b_out_t).sum()+(t_b_dist*t_out_b).sum())/denominator
