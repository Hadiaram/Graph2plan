...
def load_resplan_from_converted_pkl(pkl_path, type_to_id):
    ...
    for idx, fp in enumerate(floor_plans):
        # Each fp is a mat_struct with attributes: box, edge, boundary, etc.
        try:
            # Get room types from box data (last column is room type)
            # Access directly as fp.box (not fp.data.box for mat_struct objects)
            boxes = fp.box  # Shape: (N, 5) - [x1, y1, x2, y2, room_type]
            edges = fp.edge  # Shape: (E, 3) - [room1, room2, edge_type]
            ...
            for i in range(num_rooms):
                ...
                # For now, just use the type_id directly
                canonical = f"type_{room_type_id}"

                if canonical not in type_to_id:
                    type_to_id[canonical] = room_type_id
                ...
                # Feature: [type_id, cx, cy, 0]
                node_features.append([float(room_type_id), cx, cy, 0.0])
