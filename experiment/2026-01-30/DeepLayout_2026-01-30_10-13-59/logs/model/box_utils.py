...
def box_abs2rel(boxes,inside_boxes,obj_to_img):
    ...
    # CRITICAL FIX: Clamp denominator to prevent division by zero
...
def box_rel2abs(boxes,inside_boxes,obj_to_img):
    inside_boxes = inside_boxes[obj_to_img]
    ix0,iy0,ix1,iy1=inside_boxes[:,0],inside_boxes[:,1],inside_boxes[:,2],inside_boxes[:,3]
    # CRITICAL FIX: Clamp to prevent issues with zero-sized inside boxes
    scale_x = (ix1-ix0).clamp(min=1e-6)
    scale_y = (iy1-iy0).clamp(min=1e-6)
    xc = boxes[:,0]*scale_x+ix0
    yc = boxes[:,1]*scale_y+iy0
    w = boxes[:,2]*scale_x
    h = boxes[:,3]*scale_y
    return torch.stack([xc, yc, w, h], dim=1)
...
def apply_box_transform(anchors, transforms):
  ...
  x = xa + tx * wa
  y = ya + ty * ha
  # CRITICAL FIX: Clamp transform values to prevent exp() explosion
  tw_clamped = tw.clamp(min=-10, max=10)
...
def invert_box_transform(anchors, boxes):
  ...
  # CRITICAL FIX: Clamp to prevent division by zero and log of zero
